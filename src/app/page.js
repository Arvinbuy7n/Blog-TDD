...
export default function Ablog() {
  ...
  useEffect(() => {
    const getData = async () => {
      setIsLoading(true);
      const res = await fetch(`https://dev.to/api/articles?top=12&per_page`);
    ...
    <Stack>
      <Container className="flex justify-center">
        <Stack gap={4} className="pt-5 md:items-start flex items-center">
          <Typography fontSize={20} className="text-2xl font-bold">
            All Blog Posts
          </Typography>
          {isLoading && (
            <Typography
          ...
          <div className="hidden md:flex md:px-[500px] py-16">
            <button
              ...
              className="border-2 w-fit py-2 px-4 rounded-xl text-slate-500 hover:bg-purple-700 hover:text-white"
            >
              Load More
            </button>
          </div>
